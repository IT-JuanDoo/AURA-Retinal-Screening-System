...
public class AdminAIConfigController : ControllerBase
{
    ...
    /// <summary>
    /// Lấy chi tiết một cấu hình AI theo ID
    /// </summary>
    [HttpGet("{id}")]
    ...
    public async Task<ActionResult<AIConfigurationRowDto>> Create([FromBody] CreateAIConfigurationDto dto)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (UseDemoMode)
            {
                ...
                return Ok(new AIConfigurationRowDto(
                    $"ai-config-{Guid.NewGuid():N}",
                    dto.ConfigurationName,
                    dto.ConfigurationType,
                    dto.ModelVersionId,
                    dto.ParameterKey,
                    dto.ParameterValue,
                    dto.ParameterDataType,
                    dto.Description,
                    dto.IsActive,
                    DateTime.UtcNow,
                    GetCurrentAdminId(),
    ...
    public async Task<IActionResult> Delete(string id)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (UseDemoMode)
            {
                _logger?.LogWarning("Using demo mode for delete operation");
                return NoContent();
            }
