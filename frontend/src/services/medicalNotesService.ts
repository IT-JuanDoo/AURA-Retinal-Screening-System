...
const medicalNotesService = {
  ...
  async updateNote(
    noteId: string,
    note: Partial<CreateMedicalNoteDto>,
  ): Promise<MedicalNote> {
    const response = await api.put<MedicalNote>(
  ...
  /**
   * Get single note by ID (for current patient - dùng khi xem nhanh từ thông báo)
   */
  async getMyNoteById(noteId: string): Promise<MedicalNote> {
    const response = await api.get<MedicalNote>(
      `/medical-notes/my-notes/${noteId}`,
    );
    return response.data;
  },
  ...
  async getUnreadNotesCount(): Promise<number> {
    const response = await api.get<{ count: number }>(
      "/medical-notes/my-notes/unread-count",
    );
